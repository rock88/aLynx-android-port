...
CRom::CRom(char *romfile)
{
	...
	if((fp=fopen(mFileName,"rb"))==NULL)
	{
		...
		throw(lynxerr);
#else
			a_printf("The Lynx Boot ROM image couldn't be located!\n"
						"The lynx emulator will not run without the Boot ROM image.\n"
						"%s was not found in the lynx emulator\n"
						"directory (see the LynxEmu User Guide for more information).",romfile);
#endif
	}
	...
	if(fread(mRomData,sizeof(char),ROM_SIZE,fp)!=ROM_SIZE)
	{
		...
		throw(lynxerr);
#else
			a_printf("The Lynx Boot ROM image couldn't be loaded!");
#endif
	}
	...
	if(firsttime)
	{
		...
		for(ULONG loop=0;loop<16;loop++)
		{
			...
			{
				a_printf("FAKE LYNXBOOT.IMG - CARTRIDGES WILL NOT WORK\n\n"
