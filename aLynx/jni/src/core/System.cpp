	...
	mSusie(NULL)
{
	...
	else if(IsZip(gamefile))
	{
		...
		if((fp=(unzFile*)unzOpen(gamefile))!=NULL)
		{
			if(unzGoToFirstFile(fp)!=UNZ_OK)
			{
				...
				throw(lynxerr);
#else
			a_printf("Handy Error: ZIP File select problems");
#endif
			}
			...
			if(gotIt)
			{
				if(unzOpenCurrentFile(fp)==UNZ_OK)
				{
					...
					if(unzReadCurrentFile(fp,filememory,filesize)!=(int)info.uncompressed_size)
					{
						...
						throw(lynxerr);
#else
			a_printf("Handy Error: ZIP File load problems");
#endif
					}
	...
	else
	{
		...
		if((fp=fopen(gamefile,"rb"))==NULL)
		{
			...
			throw(lynxerr);
#else
			a_printf("Handy Error: File Open Error");
#endif
		}
	...
	if(filesize)
	{
		...
		else
		{
			...
			throw(lynxerr);
#else
			a_printf("Handy Error: File format invalid!");
#endif
		}
	}
		...
		case HANDY_FILETYPE_LNX:
			...
			if(mCart->CartHeaderLess())
			{
				...
				if((fp=fopen(cartgo,"rb"))==NULL)
				{
					...
					throw(lynxerr);
#else
					a_printf("Handy Error: Howard.o File Open Error");
#endif
				}
				...
				if(fread(howardmemory,sizeof(char),howardsize,fp)!=howardsize)
				{
					...
					throw(lynxerr);
#else
					a_printf("Handy Error: Howard.o load error (Header)");
#endif
				}
...
// If this is a snapshot type then restore the context
	...
	if(mFileType==HANDY_FILETYPE_SNAPSHOT)
	{
		if(!ContextLoad(gamefile))
		{
			...
			throw(lynxerr);
#else
			a_printf("Handy Error: Snapshot load error");
#endif
		}
	}
...
bool CSystem::ContextLoad(char *context)
{
	...
	if(IsZip(context))
	{
		...
		if((fp=(unzFile*)unzOpen(context))!=NULL)
		{
			...
			{
				unzClose(fp);
				a_printf("ContextLoad(): ZIP File select problems, could not read zip file");
				return 1;
			}
			...
			if(gotIt)
			{
				if(unzOpenCurrentFile(fp)==UNZ_OK)
				{
					...
					if(unzReadCurrentFile(fp,filememory,filesize)!=(int)info.uncompressed_size)
					{
						...
						delete filememory;
						// Throw a wobbly
						a_printf("ContextLoad(): ZIP File load problems, could not read data from the zip file");
						return 1;
					}
			...
			{
				a_printf("ContextLoad(): ZIP File load problems, could not find an LSS file in the zip archive");
				return 1;
			}
		}
		else
		{
			a_printf("ContextLoad(): ZIP File load problems, could not open the zip archive");
			return 1;
		}
	...
	if(strcmp(teststr,LSS_VERSION)==0 || strcmp(teststr,LSS_VERSION_OLD)==0)
	{
		...
		else
		{
			...
			if(mCart->CRC32()!=checksum)
			{
				delete fp;
				delete filememory;
				a_printf("LSS Snapshot CRC does not match the loaded cartridge image, aborting load");
				return 0;
			}
		}
	...
	{
		a_printf("Not a recognised LSS file");
	}
