...
//								if(mSPRSYS_VStretch) mSPRVSIZ.Word+=mSTRETCH.Word;
			...
			{
				//char message[256];
				a_printf("CSusie:PaintSprites() - Rendered Sprite %03d",sprcount);
...
//		}
		...
		if(sprcount>4096)
		{
			...
			a_printf("CSusie:PaintSprites(): Single draw sprite limit exceeded (>4096). The SCB is most likely looped back on itself. Reset/Exit is recommended");
...
//	TRACE_SUSIE1("LineInit() mLineRepeatCount=$%04x",mLineRepeatCount);
	...
	if(voff>101)
	{
		a_printf("CSusie::LineInit() Out of bounds (voff)");
		voff=0;
	}
