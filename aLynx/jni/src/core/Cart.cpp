...
CCart::CCart(UBYTE *gamedata,ULONG gamesize)
{
	...
	if(gamesize)
	{
		...
		if(header.magic[0]!='L' || header.magic[1]!='Y' || header.magic[2]!='N' || header.magic[3]!='X' || header.version!=1)
		{
			...
			throw(lynxerr);
#else			
			a_printf("Handy Error: File format invalid (Magic No)\n"
						"The image you selected was not a recognised game cartridge format.\n"
						"(see the Handy User Guide for more information).\n");
#endif
		}
	...
	switch(header.page_size_bank0)
	{
		...
		default:
			...
			throw(lynxerr);
#else
			a_printf("Handy Error: File format invalid (Bank0)\n"
					"The image you selected was not a recognised game cartridge format.\n"
					"(see the Handy User Guide for more information).");
#endif
			break;
	...
	switch(header.page_size_bank1)
	{
		...
		default:
			...
			throw(lynxerr);
#else
			a_printf("Handy Error: File format invalid (Bank1)\n"
					"The image you selected was not a recognised game cartridge format.\n"
					"(see the Handy User Guide for more information).");
#endif
			break;
